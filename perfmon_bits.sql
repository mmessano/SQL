...
select distinct count(*) AS Occurences, convert(char(10), Date, 101) AS Date
from CountersDataDaily
...
select * from CountersDataDaily order by Date,lastupdate desc
select * from CountersDisplay where DisplayString like 'Apollo%'

select * from CountersData where serverid = (select server_id from newton.Status.dbo.t_server where server_name = 'apollo')
and CounterDateTime > GetDate() - 2
order by CounterDateTime desc


exec PerfmonTransform