...
GROUP BY InstanceID WITH ROLLUP
--------------------------------
USE PA_DMart
GO
SELECT SourceDB
...
--------------------------------
-- works, no client sub-totals
--SELECT CASE WHEN SourceDB IS NULL THEN 'Grand Total' ELSE SourceDB END SourceDB
--		, SUM(Beta) AS Beta
--		, CONVERT(TIME,DATEADD(ms, SUM(StageLoadTime), 0)) AS StageLoadTime
--		, CONVERT(TIME,DATEADD(ms, SUM(ReportLoadTime), 0)) AS ReportLoadTime
--FROM (		
--SELECT SourceDB
--      , SourceServer
--      , LEFT(REVERSE(SourceServer), 1) AS SQLServer
--      , Beta
--      , CONVERT(INT, DATEDIFF(ms, LoadStageDBStartDate, LoadStageDBEndDate)) AS StageLoadTime
--      , CONVERT(INT, DATEDIFF(ms, LoadReportDBStartDate, LoadReportDBEndDate)) AS ReportLoadTime
--FROM    ClientConnection
--GROUP BY Beta, Status, SourceDB, SourceServer, LoadStageDBStartDate, LoadStageDBEndDate, LoadReportDBStartDate, LoadReportDBEndDate
--) AS Time
--GROUP BY Time.SourceDB WITH ROLLUP
--ORDER BY Beta ASC, SourceDB ASC
