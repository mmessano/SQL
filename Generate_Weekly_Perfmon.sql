...
FROM
	...
	newton.Status.dbo.t_server s ON s.server_id = cdd.ServerID
where  convert(char(10), Date, 101) > GetDate() - 3
--where  convert(char(10), Date, 101) = (GetDate() - 6)
	GROUP BY  s.server_id, c.CountersID, convert(char(10), Date, 101)
) AS f
order by server_id, CountersID, Day
